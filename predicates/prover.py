...
class Prover:
    ...
    def add_mp(
        ...
        return self._add_line(
            Proof.MPLine(
                consequent, antecedent_line_number, conditional_line_number
            )
        )
    ...
    def add_ug(
        ...
        return self._add_line(
            Proof.UGLine(quantified, nonquantified_line_number)
        )
    ...
    def add_proof(self, conclusion: Union[Formula, str], proof: Proof) -> int:
        ...
        for line in proof.lines:
            if type(line) in {Proof.AssumptionLine, Proof.TautologyLine}:
                ...
                self.add_ug(
                    line.formula, line.nonquantified_line_number + line_shift
                )
    ...
    def add_universal_instantiation(
        ...
        quantified: Formula = self._lines[line_number].formula
        ...
        mp_first: Formula = Formula("->", quantified, instantiation)

        applied_ui_line_num: int = self.add_instantiated_assumption(
            mp_first,
            Prover.UI,
            {
                "R": quantified.statement.substitute(  # type: ignore
                    {quantified.variable: Term("_")}  # type: ignore
                ),
                "x": quantified.variable,  # type: ignore
                "c": term,
            },
        )
    ...
    def add_free_instantiation(
        ...
        for variable in substitution_map:
            ...
            for variable in term.variables():
                assert variable[0] != "z"
        assert instantiation == self._lines[line_number].formula.substitute(
            substitution_map
        )
