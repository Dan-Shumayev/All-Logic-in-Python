...
def remove_assumption(
    ...
    for i, line in enumerate(proof.lines):
        if isinstance(line, Proof.TautologyLine) or line.formula == phi:
            line_in_new_proof[i] = prover.add_tautology(
                f"({phi}->{line.formula})"
            )
            ...
            step1 = prover.add_instantiated_assumption(
                alpha, line.assumption, line.instantiation_map
            )
            step2 = prover.add_tautology(f"({alpha}->({phi}->{alpha}))")
            line_in_new_proof[i] = prover.add_mp(
                f"({phi}->{alpha})", step1, step2
            )
            ...
            step2 = prover.add_mp(
                f"(({phi}->{alpha})->({phi}->{beta}))",
                line_in_new_proof[line.conditional_line_number],
                step1,
            )
            line_in_new_proof[i] = prover.add_mp(
                f"({phi}->{beta})",
                line_in_new_proof[line.antecedent_line_number],
                step2,
            )
            ...
            step1 = prover.add_ug(
                f"A{x}[({phi}->{alpha})]",
                line_in_new_proof[line.nonquantified_line_number],
            )
            us_inst_mapping = {
                "Q": phi,
            ...
            step2 = prover.add_instantiated_assumption(
                ins, Prover.US, us_inst_mapping
            )
            line_in_new_proof[i] = prover.add_mp(
                f"({phi}->A{x}[{alpha}])", step1, step2
            )
