...
def __test_synthesize(synthesizer, dnf, debug):
    ...
    for all_variables, all_models, value_lists in [
        ...
        for all_values in value_lists:
            ...
            assert type(formula) is Formula, "Expected a formula, got " + str(
                formula
            )
            if dnf:
...
def test_evaluate_inference(debug=False):
    ...
    for model in all_models(["p", "q", "r"]):
        if debug:
            print(
                "Testing evaluation of inference rule", rule1, "in model", model
            )
        assert (
    ...
    for model in all_models(["x", "y"]):
        if debug:
            print(
                "Testing evaluation of inference rule", rule2, "in model", model
            )
        assert (
    ...
    for model in all_models(["p", "q", "r"]):
        if debug:
            print(
                "Testing evaluation of inference rule", rule3, "in model", model
            )
        assert (
