...
def test_is_line_valid(debug=False):
    ...
    if debug:
        print(
            "\nChecking proof line vailidity in proof of",
            lemma,
            "using rules",
            {rule0, rule1},
        )
...
def test_prove_specialization(debug=False):
    ...
    for instance_infix in [
        ...
        assert instance_proof.statement == instance
        assert (
            instance_proof.rules == DISJUNCTION_RIGHT_ASSOCIATIVITY_PROOF.rules
        )
        assert instance_proof.is_valid(), offending_line(instance_proof)
...
def test_inline_proof_once(debug=False):
    ...
    rule0 = InferenceRule(
        [Formula.parse("((x|y)|z)")], Formula.parse("(x|(y|z))")
    )
    # Disjunction commutativity with an unused assumption
    rule1 = InferenceRule(
        [Formula.parse("(~q|q)"), Formula.parse("(x|y)")],
        Formula.parse("(y|x)"),
    )
    ...
    proof = Proof(
        InferenceRule([Formula.parse("((p|q)|r)")], Formula.parse("((r|p)|q)")),
        {rule0, rule1, rule2},
        [
    ...
    if debug:
        print(
            "Testing _inline_proof_once (test 1). In main proof:\n",
            proof,
            "Replacing line",
            line_number,
    ...
    if debug:
        print(
            "Testing _inline_proof_once (test 2). In main proof:\n",
            proof,
            "Replacing line",
            line_number,
            "with the proof of following lemma proof:\n",
    ...
    statement = InferenceRule(
        [Formula.parse("(x&y)"), Formula.parse("(w&z)")],
    ...
    if debug:
        print(
            "Testing _inline_proof_once (final). In main proof:\n",
            proof,
            "Replacing line",
            line_number,
            "with the proof of following lemma proof:\n",
...
def test_inline_proof(debug=False):
    ...
    if debug:
        print(
            "Testing inline_proof (#1) for the following main proof:\n"
            + str(proof)
    ...
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == proof.rules.union(
        lemma2_proof.rules
    ).difference({lemma2_proof.statement}), "Rule are: " + str(
        inlined_proof.rules
    )
    assert inlined_proof.is_valid(), offending_line(inlined_proof)
    ...
    if debug:
        print(
            "Testing inline_proof (#2) for the following main proof:\n"
            + str(inlined_proof)
            + "\nand the following lemma proof:\n"
            + str(lemma1_proof)
        )
    ...
    assert inlined_proof.rules == proof.rules.union(
        lemma2_proof.rules
    ).difference({lemma2_proof.statement}).union(lemma1_proof.rules).difference(
    ...
    if debug:
        print(
            "Testing inline_proof (#3) for the following main proof:\n"
            + str(proof)
            + "\nand the following lemma proof:\n"
            + str(lemma1_proof)
        )
    ...
    if debug:
        print(
            "Testing inline_proof (#4) for the following main proof:\n"
            + str(inlined_proof)
            + "\nand the following lemma proof:\n"
            + str(lemma2_proof)
        )
    ...
    assert inlined_proof.rules == proof.rules.union(
        lemma1_proof.rules
    ).difference({lemma1_proof.statement}).union(lemma2_proof.rules).difference(
    ...
    if debug:
        print(
            "Testing inline_proof (#5) for the following main proof:\n"
            + str(lemma2_proof)
            + "\nand the following lemma proof:\n",
            str(lemma1_proof),
        )
    ...
    assert inlined_lemma.rules == lemma1_proof.rules.union(
        lemma2_proof.rules
    ).difference({lemma1_proof.statement})
    ...
    if debug:
        print(
            "Testing inline_proof (#6) for the following main proof:\n"
            + str(inlined_proof)
            + "\nand the following lemma proof:\n"
            + str(inlined_lemma)
        )
    ...
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == inlined_lemma.rules.union(
        inlined_proof.rules
